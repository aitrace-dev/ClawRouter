 ...
 *   1. Environment variables (GOOGLE_API_KEY, ANTHROPIC_API_KEY, etc.)
...
/**
 * Providers that accept the OpenAI /v1/chat/completions format natively.
 */
...
/**
 * Providers that can be called directly (with or without format conversion).
 * Includes OPENAI_COMPATIBLE_PROVIDERS plus providers with dedicated handling
 * in the proxy layer.
 */
const DIRECT_ACCESS_PROVIDERS = new Set([
  ...OPENAI_COMPATIBLE_PROVIDERS,
  "google",    // Uses OpenAI-compatible endpoint (/v1beta/openai/)
  "anthropic", // Proxy converts to Anthropic format (convertToAnthropicFormat)
]);
  ...
  for (const [provider, envVars] of Object.entries(ENV_VAR_MAP)) {
    for (const envVar of envVars) {
      ...
      if (key) {
        ...
        break; // Use first found env var for this provider
      }
    }
  }
...
/**
 * Get the base URL for a provider (custom or default).
 */
...
/**
 * Resolve the best API key + base URL for a model.
...
export function resolveProviderAccess(
  ...
  // Anthropic + Google can go via OpenRouter if available (handles format conversion)
  ...
  //    Works for OpenAI-compatible providers, Google (via /v1beta/openai/ endpoint),
  //    and Anthropic (via format conversion in proxy layer)
  const directKey = getApiKey(config, provider);
  const directUrl = getProviderBaseUrl(config, provider);
  if (directKey && directUrl && DIRECT_ACCESS_PROVIDERS.has(provider)) {
