...
function buildUpstreamUrl(
  ...
  // Google's OpenAI-compatible endpoint accepts standard /chat/completions format
  if (provider === "google") {
    const googlePath = baseUrl.endsWith("/openai") && path.startsWith("/v1")
      ? path.slice(3) // strip /v1 prefix since baseUrl already includes /openai
      : path;
    return {
      url: `${baseUrl}${googlePath}`,
      provider,
      apiKey,
      actualModelId,
      viaOpenRouter: false,
...
function buildProviderHeaders(provider: string, apiKey: string, viaOpenRouter = false): Record<string, string> {
  ...
  switch (provider) {
    case "anthropic":
      ...
      break;
    case "google":
      // Google's OpenAI-compatible endpoint accepts Bearer auth
      headers["authorization"] = `Bearer ${apiKey}`;
      break;
    default:
